...
// Used at step 1 onAdvance to update the split row + anchor→split lines
// after the child picks the ones digit. Does NOT render the bottom row
// or split→bottom merge lines — those defer to step 2's audio
// onComplete so they only appear AFTER the ones-addition audio
// ("个位相加 [ones] 加 [b] 等于几") finishes. Per user feedback
// 2026-08-15: "在小孩进行了第一步10的拆解，把12拆解完10+2之后，不应该
// 里面出现10+？=？这个和合并线。应该在播放完个位相加播放完了之后，
// 再出现。"
function renderL4Step1SplitComplete(ctx, round, ones) {
...
export default createRoundScene({
  ...
  steps: [
    ...
    (ctx, round) => {
      ...
      return {
        ...
        onAdvance: () => {
          ...
          // bottom row (10+□=?) and its merge lines are deferred to
          // step 2's audio onComplete so they don't pop up while the
          // child is still parsing the split result. See
          // renderL4Step1SplitComplete's note.
          renderL4Step1SplitComplete(ctx, round, ones);
        },
      };
    },
    ...
    (ctx, round) => {
      ...
      // the equation they're being asked to compute appears. Per user
      // feedback 2026-08-15. Buttons render normally via buildStep
      // below; only the equation + merge lines are gated on audio.
      fireL3StepAudio(
        ctx, buildL3Step2Ids(ones, round.b), 2,
