...
async function playOneRound(roundNum) {
  ...
  await page.waitForTimeout(SKIP_TIMERS ? 500 : 3000);
  return true;
}
...
for (let r = 1; r <= 3; r++) {
  ...
  await page.waitForTimeout(SKIP_TIMERS ? 300 : 2000);
}
log(`=== probe complete, page should be alive ===`);
await page.waitForTimeout(SKIP_TIMERS ? 500 : 2000);
log(`=== ALL MESSAGES ===`);
