// Same as probe-freeze2.mjs but sets window.__trace_no_eat = true
import { chromium } from "playwright";
const URL = process.env.PANDA_URL || "http://localhost:8126/";
const SKIP_TIMERS = process.env.SKIP_TIMERS !== "0";
const TRACE_NO_EAT = process.env.TRACE_NO_EAT !== "0";
console.log(`[probe] SKIP_TIMERS=${SKIP_TIMERS} TRACE_NO_EAT=${TRACE_NO_EAT}`);
...
await page.addInitScript(({skipTimers, traceNoEat}) => {
  window.__skipTimers = skipTimers;
  window.__trace_no_eat = traceNoEat;
  window.__trace_no_audio = traceNoEat;  // pairScene's flag
}, { skipTimers: SKIP_TIMERS, traceNoEat: TRACE_NO_EAT });
...
async function playOneRound(roundNum) {
  ...
  for (let i = 0; i < vals.length; i++) {
    for (let j = i + 1; j < vals.length; j++) {
      if (vals[i] + vals[j] === Number(target)) { pair = [bubbles[i], bubbles[j]]; break; }
    }
    if (pair) break;
  }
  if (!pair) { log(`no pair for ${target}, abort`); return false; }
  log(`clicking (${pair[0].value}, ${pair[1].value}) at (${pair[0].x}, 624) & (${pair[1].x}, 624)`);
  try { await page.mouse.click(pair[0].x, 624, { timeout: 3000 }); } catch (e) { log(`click 1 failed: ${e.message}`); return false; }
  await page.waitForTimeout(80);
  try { await page.mouse.click(pair[1].x, 624, { timeout: 3000 }); } catch (e) { log(`click 2 failed: ${e.message}`); return false; }
